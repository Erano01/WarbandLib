...
std::string Sha256Hex(const std::uint8_t* data, std::size_t len) {
	...
	// Pad message: 0x80, zeros, then 64-bit big-endian bit length, to a
	// multiple of 64 bytes.
	std::uint64_t bit_len = static_cast<std::uint64_t>(len) * 8;
	std::size_t padded_len = ((len + 9 + 63) / 64) * 64;
	std::vector<std::uint8_t> msg(padded_len, 0);
	std::memcpy(msg.data(), data, len);
	msg[len] = 0x80;
	for (int i = 0; i < 8; ++i) {
		msg[padded_len - 1 - i] = static_cast<std::uint8_t>(bit_len >> (8 * i));
	}
