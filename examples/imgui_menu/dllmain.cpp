// Example: a real ImGui window drawn via runtime/view/, toggled with F10.
// Deliberately a separate, standalone DLL from warbandlib_runtime.dll and
// from examples/overlay_quad -- to disable it, simply don't inject it (or
// close the game to unload it); it never touches the other DLLs' state.
...
// Same target exe as the main runtime and examples/overlay_quad. See
...
DWORD WINAPI WorkerThread(LPVOID) {
	...
	g_logger = std::make_unique<Logger>(dir + "WarbandLibExampleImguiMenu.log");
	g_logger->Write("imgui_menu example attached, waiting for D3D device...");
	...
	ViewLayer::Configure(g_logger.get(), &warbandlib::examples::imgui_menu::DrawMenu);
